...
def _migrate_db():
    """Add face_embedding column to existing tables if missing."""
    conn = sqlite3.connect("sqlite_database.db")
    cursor = conn.cursor()
    for table in ["publicsubmissions", "registeredcases"]:
        try:
            cursor.execute(f"ALTER TABLE {table} ADD COLUMN face_embedding TEXT")
...
def reset_database():
    try:
        ...
        RegisteredCases.__table__.drop(engine)
        PublicSubmissions.__table__.drop(engine)
        create_db()
        return True
